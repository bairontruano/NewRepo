...
public:
	...
	void initialize_textures(void)
	{
		...
		FIBITMAP* bitmap = FreeImage_Load(
			FreeImage_GetFileType("./Mallas/piedraO.jpg", 0),
			"./Mallas/piedraO.jpg");  //*** Para Textura: esta es la ruta en donde se encuentra la textura
		...
		//	FreeImage_GetFileType("./Mallas/WoodCabinDif.jpg", 0),
		//	"./Mallas/WoodCabinDif.jpg");  //*** Para Textura: esta es la ruta en donde se encuentra la textura

		//FIBITMAP* pImage2 = FreeImage_ConvertTo32Bits(bitmap2);
		//int nWidth1 = FreeImage_GetWidth(pImage2);
		//int nHeight1 = FreeImage_GetHeight(pImage2);

		//glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, nWidth1, nHeight1,
	...
	virtual void OnRender(void)
	{
      ...
      if (shader) shader->begin();
		  ...
		  glmDraw(objmodel_ptrM, GLM_SMOOTH | GLM_MATERIAL);
		  glPopMatrix();
	      //glutSolidTeapot(1.0);
	...
	virtual void OnInit()
	{
	 ...
	 //*** Para Textura: abre los shaders para texturas
	  ...
	  if (!objmodel_ptrM)
	  {
		  objmodel_ptrM = glmReadOBJ("./Mallas/asteroidobj.obj");
		  if (!objmodel_ptrM)
			  exit(0);

		  glmUnitize(objmodel_ptrM);
		  glmFacetNormals(objmodel_ptrM);
		  glmVertexNormals(objmodel_ptrM, 90.0);
	  }
	  ...
	  if (!objmodel_ptr1)
	  {
		  objmodel_ptr1 = glmReadOBJ("./Mallas/piedraO.obj");
		  if (!objmodel_ptr1)
			  exit(0);
	  ...
	  if (!objmodel_ptr2)
	  {
		  objmodel_ptr2 = glmReadOBJ("./Mallas/WoodenCabinBlender.obj");
		  if (!objmodel_ptr2)
			  exit(0);
